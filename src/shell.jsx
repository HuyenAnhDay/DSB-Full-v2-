...
import { COMPASSES, SECTIONS, NAV, ADVISOR, ANNOUNCEMENTS } from './data.js';
...
function AppShell({ route, onNavigate, compact, setCompact, children }) {
  ...
  const compass = SECTIONS.find(c => c.id === compassId) || SECTIONS[1];
  const subNav = NAV[compass.id] || [];
  const flatNav = subNav.flatMap(e => e.items ? e.items : [e]);
  const activeScreen = screenId ? (compass.id + "/" + screenId) : (flatNav[0]?.id || compass.id);
...
const DASHBOARD_NAV = { id: "dashboard", label: "Dashboard", icon: "dashboard", sub: "Tổng quan toàn hệ thống" };
const UTILITY_NAV = [
  { id: "toolbox",    label: "Toolbox", icon: "build",     sub: "Agenda · Simulator" },
  { id: "library/kb", label: "Library", icon: "menu_book", sub: "KB Note · Helpdesk" }
];
...
function SecondaryNav({ compass, subNav, active, onNavigate }) {
  ...
  const groupLabel = (text) => (
    <span className="text-[10.5px] uppercase tracking-[0.12em] font-bold text-on-surface-variant/45 flex items-center h-full select-none">
      {text}
    </span>
  );
  return (
    <nav className="bg-surface-container-lowest border-b border-outline-variant/30 sticky top-16 z-10 overflow-x-auto whitespace-nowrap scrollbar-hide">
      <div className="max-w-[1440px] mx-auto px-gutter flex items-center h-12 gap-unit-6">
        {subNav.map((entry, i) => {
          const divider = i > 0 ? <div className="w-px h-4 bg-outline-variant/80"></div> : null;
          // Group entry
          if (entry.items) {
            // Collapse single-item group whose item is the group itself (OC, IPAX…)
            if (entry.items.length === 1 && entry.items[0].label === entry.group) {
              return (
                <span key={entry.group} className="contents">
                  {divider}{renderTab(entry.items[0])}
                </span>
              );
            }
            return (
...
export { AppShell, Sidebar, TopBar, SecondaryNav, PageHeader, Page, AIBuddyPopover, mockReply, AI_SUGGESTIONS };
